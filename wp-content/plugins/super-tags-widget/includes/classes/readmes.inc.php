...
/**/
if (!class_exists ("c_ws_widget__super_tags_readmes"))
	{
		class c_ws_widget__super_tags_readmes
			{
				/*
				...
				public static function parse_readme ($specific_path = FALSE, $specific_section = FALSE, $_blank_targets = TRUE, $process_wp_syntax = FALSE)
					{
						...
						do_action ("ws_widget__super_tags_before_parse_readme", get_defined_vars ());
								...
								if ($specific_section) /* If we are ONLY parsing a specific section. This is a very useful way of pulling details out. */
									{
										...
										$readme .= '</div>' . "\n";
										/**/
										return apply_filters ("ws_widget__super_tags_parse_readme", $readme, get_defined_vars ());
									}
								else /* Otherwise, we're going for the entire readme file. Here we have lots of work to do. */
									{
										...
										$rm = stripslashes (preg_replace ($r3, "'$1$2$3$4$5'.c_ws_widget__super_tags_readmes::_parse_readme_specs('$6').'$7'", $rm, 1));
				...
				*/
				public static function _parse_readme_specs ($str = FALSE)
					{
						do_action ("_ws_widget__super_tags_before_parse_readme_specs", get_defined_vars ());
						/**/
						...
						/**/
						return apply_filters ("_ws_widget__super_tags_parse_readme_specs", $str, get_defined_vars ());
					}
				/*
				Function for parsing readme.txt files and returning a key value.
				*/
				public static function readme_value ($key = FALSE, $specific_path = FALSE)
					{
						...
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_widget__super_tags_before_parse_readme_value", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if ($readme[$path] || file_exists ($path))
							{
								...
								/**/
								return strlen ($m[5] = trim ($m[5])) ? apply_filters ("ws_widget__super_tags_parse_readme_value", $m[5], get_defined_vars ()) : false;
							}
						else /* Nope. */
							return false;
					}
